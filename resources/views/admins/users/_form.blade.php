...
<div class="card card-purple">
	...
	<!-- /.card-header -->
	<form method="POST" action="{{ $isEdit ? route('users.update', $user->staff_id) : route('users.store') }}">
		{{ $isEdit ? method_field('PUT') : '' }}
		@csrf
		<div class="card-body row">
			<div class="form-group col-md-6">
				<label for="input-full_name">Full Name</label>
				<input type="text" class="form-control @error('full_name') is-invalid @enderror" placeholder="Full Name" id="input-full_name" name="full_name" value="{{ old('full_name', $user->info->full_name) }}" required autofocus>
			...
			<div class="form-group col-md-6">
				...
				<input type="email" class="form-control @error('email') is-invalid @enderror" placeholder="Email" id="input-email" name="email" value="{{ old('email', $user->info->email) }}" required>
				<!-- sent message error input -->
				@error('email') 
				<span class="invalid-feedback" role="alert">
					<strong> *{{ $message }} </strong>
